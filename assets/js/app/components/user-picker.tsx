...
export function UserPicker() {
  ...
  const buttonState: "loading" | "user" | "no-user" = isCurrentUserLoading
    ? "loading"
    : currentUser
      ? "user"
      : "no-user";
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      >
        {buttonState === "loading" ? (
          <>
            <span className="w-7 h-7 rounded-full bg-base-200 animate-pulse" />
            <span className="hidden sm:inline w-24 h-3 rounded bg-base-200 animate-pulse" />
          </>
        ) : buttonState === "user" && currentUser ? (
          <>
          ...
          <span className="whitespace-nowrap">Pick a user</span>
        )}
        <svg
          className={`w-3.5 h-3.5 transition-transform duration-200 ${buttonState === "no-user" ? "text-base-100/70" : "text-base-content/60"
            } ${open ? "rotate-180" : ""}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
              ...
              <li key={user.id}>
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div
                      className={`text-xs capitalize ${currentUser?.id === user.id
